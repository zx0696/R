...
# 패키지명::데이터명 -> 패키지 내에 저장되어 있는 데이터를 불러온다
...
mpg_b <- mpg %>% filter(displ < 5) %>% summarise(mean_less5 = mean(hwy))
...
new_mpg <- new_mpg %>% mutate(합산연비 = cty+hwy,
                              평균연비 = 합산연비/2) # 1,2번 같이 처리

# 3.
#'평균 연비 변수'가 가장 높은 자동차 3 종의 데이터를 출력하세요. 
new_mpg %>% arrange(desc(평균연비)) %>% 
  head(3)
...
mpg %>% mutate(합산연비=cty+hwy,
               평균연비=합산연비/2) %>% 
  arrange(desc(평균연비)) %>% 
  head(3) %>% select(manufacturer,model,평균연비)
...
mpg %>% group_by(manufacturer) %>% # 회사별로
  ...
  arrange(desc(mean_tot)) %>% # 산출된 연비평균으로 내림차순 정렬
  head(5) # 상위 5개 정보 출력
...
mpg %>% filter(class=="compact") %>% # compact 추출 
  group_by(manufacturer, model) %>%  # 제조사별로 그룹
